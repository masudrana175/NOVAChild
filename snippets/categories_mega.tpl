{block name='snippets-categories-mega'}
    ...
    {if $Einstellungen.template.megamenu.show_categories !== 'N'
        ...
        {get_category_array categoryId=startId assign='categoryTemp'}

        {foreach name="categoriesFirstLayerTemp" from=$categoryTemp item=categoryTempItem key=k}
            {if $k === $startId}
                {assign var="categories" value=$categoryTempItem->getChildren()}
            {/if}
        {/foreach}
        ...
        {* ## Kategorie Array ## *}
            ...
            {* {get_category_array categoryId=$startId assign='categories'} *}
                            ...
                            || (isset($activeParent)
                                ...
                                {link href=$category->getURL()
                                    ...
                                    {if $isMobile}
                                        ...
                                        srcSize='xs'
                                        width= '100px'
                                        alt="{if empty($imgAlt->cWert)}{$category->getName()|@seofy}{else}{$imgAlt->cWert}{/if}"}
                                    {/if}
                            ...
                            <li class="nav-item nav-scrollbar-item
                                ...
                                {link href=$category->getURL()
                                    ...
                                    {if $isMobile}
                                        ...
                                        alt="{if empty($imgAlt->cWert)}{$category->getName()|@seofy}{else}{$imgAlt->cWert}{/if}"}
                                    {/if}
                                    <span class="nav-mobile-heading text-truncate d-block">{$category->getShortName()}</span>
                                {/link}
                            </li>
                        {/block}
                    {/if}
