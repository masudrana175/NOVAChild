...
{block name='basket-index-price-tax'}
    {* One-Page-Checkout-Vorschau (nur Testkreis) per Geheim-Link aktivieren/deaktivieren.
       Aktivieren:   <Shop-URL>/Warenkorb?opc=beautek-opc-2026   (setzt Cookie, bleibt aktiv)
       Deaktivieren: <Shop-URL>/Warenkorb?opc=off
       Das Cookie steuert anschließend die Sichtbarkeit der neuen Ansicht im Checkout. *}
    {if isset($smarty.get.opc)}
        <script>
        (function () {
            try {
                var on = {if $smarty.get.opc === 'beautek-opc-2026'}true{else}false{/if};
                if (on) { document.cookie = 'beautek_opc=1;path=/;max-age=2592000'; }
                else { document.cookie = 'beautek_opc=;path=/;max-age=0'; }
            } catch (e) {}
        })();
        </script>
        {if $smarty.get.opc === 'beautek-opc-2026'}
            <div class="beautek-opc-flag">One-Page-Checkout-Vorschau aktiviert (nur Testkreis)</div>
        {/if}
    {/if}
