{block name='productdetails-price'}
    {if $Artikel->Preise !== null && $smarty.session.Kundengruppe->mayViewPrices()}
        <div class="price_wrapper custom-price">
            {$basePriceUnit = $Artikel->cVPEEinheit|default:''|regex_replace:"/^[\d\s]+/":""|trim}
            {$basePriceLocalized = $Artikel->cLocalizedVPE[$NettoPreise]|default:''}
            {if $basePriceLocalized !== '' && $basePriceUnit !== '' && ($basePriceLocalized|strpos:$basePriceUnit) === false}
                {$basePriceLocalized = $basePriceLocalized|cat:' '|cat:$basePriceUnit}
            {/if}
            {if $basePriceLocalized === '' && $Artikel->fVPEWert > 0 && $basePriceUnit !== ''}
                {if $Artikel->Preise->oPriceRange->isRange()}
                    {if $NettoPreise == 1}
                        {$basePriceValue = $Artikel->Preise->oPriceRange->minNettoPrice / $Artikel->fVPEWert}
                    {else}
                        {$basePriceValue = $Artikel->Preise->oPriceRange->minBruttoPrice / $Artikel->fVPEWert}
                    {/if}
                {else}
                    {$basePriceValue = $Artikel->Preise->fVK[$NettoPreise] / $Artikel->fVPEWert}
                {/if}
                {$basePriceLocalized = $basePriceValue|string_format:"%.2f"|replace:".":","|cat:' '|cat:$smarty.session.Waehrung->htmlEntity|cat:' pro '|cat:$Artikel->fGrundpreisMenge|cat:' '|cat:$basePriceUnit}
            {/if}
                                ...
                                {if $basePriceLocalized !== ''}
                                    <span itemprop="priceSpecification" itemscope itemtype="https://schema.org/UnitPriceSpecification">
                                        ...
                                        <span itemprop="referenceQuantity" itemscope itemtype="https://schema.org/QuantitativeValue">
                                            <meta itemprop="price" content="{$basePriceLocalized}">
            ...
            {else}
                ...
                {if $tplscope === 'detail'}
                    ...
                    {block name='productdetails-price-special-prices-detail'}
                        {if $Artikel->Preise->Sonderpreis_aktiv && $Einstellungen.artikeldetails.artikeldetails_sonderpreisanzeige == 2 || !$Artikel->Preise->Sonderpreis_aktiv && $Artikel->Preise->rabatt > 0}
                            <div class="special-price">
                                {if $Artikel->Preise->rabatt != 0}
                                    {assign var="priceDiffPer" value=$Artikel->Preise->rabatt}
                                    {* {assign var="priceDiffPer" value=12.00} *}

                                    {if $Artikel->Preise->oPriceRange->isRange() && ($Artikel->nVariationsAufpreisVorhanden == 1 || $Artikel->bHasKonfig) && $Artikel->kVaterArtikel == 0}

                                        {assign var="priceDiff" value=$Artikel->Preise->oPriceRange->getMinLocalized($NettoPreise) / 100 * $priceDiffPer}

                                    {else}

                                        {assign var="priceDiff" value=$Artikel->Preise->cVKLocalized[$NettoPreise] / 100 * $priceDiffPer}

                                    {/if}

                                    {assign var="priceDiff" value=$priceDiff|string_format:"%.2f"}
                                {else}

                                    {assign var="priceDiff" value=$Artikel->Preise->alterVKNetto - $Artikel->Preise->fVKNetto}

                                {/if}

                                {* <pre>{$Artikel->Preise|var_dump}</pre>
                                <pre>{$Artikel->Preise->cVKLocalized[$NettoPreise]|var_dump}</pre> *}


                                {* {if $Artikel->Preise->Sonderpreis_aktiv && $Einstellungen.artikeldetails.artikeldetails_sonderpreisanzeige == 2 || !$Artikel->Preise->Sonderpreis_aktiv && $Artikel->Preise->rabatt > 0} *}
                                    <div class="discount">
                                        <span class="value">Du sparst: <strong>{if $Artikel->Preise->oPriceRange->isRange() && ($Artikel->nVariationsAufpreisVorhanden == 1 || $Artikel->bHasKonfig) && $Artikel->kVaterArtikel == 0} {/if}{$priceDiff|replace:".":","}{$smarty.session.Waehrung->htmlEntity}</strong></span>
                                    </div>
                ...
                {if $tplscope === 'detail'}
                    {block name='productdetails-price-detail'}
                        <div class="price-note">
                            ...
                            {* <pre>{$Artikel->Preise|var_dump}</pre> *}
                            {* <pre>{$Artikel->Preise->cVKLocalized|var_dump}</pre> *}
                            {if $NettoPreise == '1'}
                                <div class="price-brutto">

                                    <span class="price-brutto_starting">{lang key="gross"}: </span>

                                    {if $Artikel->Preise->oPriceRange->isRange()}
                                        {if $Artikel->Preise->oPriceRange->rangeWidth() <= $Einstellungen.artikeluebersicht.articleoverview_pricerange_width}
                                            {assign var=rangePrices value=$Artikel->Preise->oPriceRange->getLocalizedArray(0)}
                                            <span class="first-range-price">{$rangePrices[0]} - </span><span class="second-range-price">{$rangePrices[1]} {if $tplscope !== 'detail'} <span class="footnote-reference">*</span>{/if}</span>
                                        {else}
                                            {$Artikel->Preise->oPriceRange->getMinLocalized(0)} <span class="footnote-reference">*</span>
                                        {/if}
                                    {else}
                                        {if $Artikel->Preise->oPriceRange->isRange() && ($Artikel->nVariationsAufpreisVorhanden == 1 || $Artikel->bHasKonfig) && $Artikel->kVaterArtikel == 0}{$Artikel->Preise->oPriceRange->getMinLocalized(0)}{else}{$Artikel->Preise->cVKLocalized[0]}{/if}
                                    {/if}
                    ...
                    <div class="price-note">
                        ...
                        {block name='productdetails-price-special-prices'}
                            {if $Artikel->Preise->Sonderpreis_aktiv && isset($Einstellungen.artikeluebersicht) && $Einstellungen.artikeluebersicht.artikeluebersicht_sonderpreisanzeige == 2}
                                <span class="old-price-inline" aria-label="{lang key='oldPrice'}">
