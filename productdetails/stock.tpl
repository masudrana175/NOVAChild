{block name='productdetails-stock'}
    ...
    {block name='productdetails-stock-stock-info'}
        ...
        {if $Artikel->cEstimatedDelivery == NULL}
            <span class="ml-3">{lang key="shipmentPreVariationChoice" section="custom"}</span>
        {else}
        ...
        {* Wunschliefertermin-Hinweis nur bei Speditionssendungen (NICHT bei Paketversand) *}
        {if (isset($Artikel->cVersandklasse) && $Artikel->cVersandklasse|strpos:'Paketversand' === false && $Artikel->cVersandklasse != '') 
            || $Artikel->fGewicht >= 31.5}
        <div class="wunschlieferzeit-hinweis">
            Wunschliefertermin? Schreibe es in den Bestellkommentar oder <a href="tel:+4927131374330">ruf uns an</a>.
        </div>
        {/if}
        {/if}
    {/block}
