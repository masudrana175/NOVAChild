        ...
        || $useAvailabilityNotification || $useMediaGroup || !empty($separatedTabs)}
            ...
            {if $tabanzeige}
                {block name='productdetails-tabs-no-tabs'}
                    {container class="cstm-accordion-wrapper container {if $Einstellungen.template.theme.left_sidebar === 'Y' && $boxesLeftActive}container-plus-sidebar{/if}"}
                        <div class="accordion tabAccordionCstm" id="tabAccordion"{$quickViewIdPostfix}>
                            ...
                            {if (isset($gpsr_manufacturer) && $gpsr_manufacturer|@count > 0) || (isset($gpsr_responsibleperson) && $gpsr_responsibleperson|@count > 0)}
                                 {card no-body=true}
                                         {cardheader id="tab-sicherheitshinweise-head"|cat:$quickViewIdPostfix
                                             data=["toggle" => "collapse",
                                                 "target"=>"#tab-sicherheitshinweise"|cat:$quickViewIdPostfix
                                             ]
                                             aria=["expanded" => "{if $setActiveClass.sicherheitshinweise}true{else}false{/if}",
                                                 "controls" => "tab-sicherheitshinweise"|cat:$quickViewIdPostfix
                                             ]
                                         }
                                         {lang section='custom' key='tab_sicherheitshinweise'}
                                         {/cardheader}
                                     {collapse id="tab-sicherheitshinweise"|cat:$quickViewIdPostfix
                                         visible=$setActiveClass.downloads
                                         data=["parent"=>"#tabAccordion"|cat:$quickViewIdPostfix]
                                         aria=["labelledby"=>"tab-sicherheitshinweise-head"|cat:$quickViewIdPostfix]
                                     }
                                         {cardbody}
                                             {opcMountPoint id='opc_before_sicherheit'}
                                             {include file='productdetails/gpsr.tpl'}
                                             {opcMountPoint id='opc_after_sicherheit'}
                                         {/cardbody}
                                     {/collapse}
                                 {/card}
                             {/if}
