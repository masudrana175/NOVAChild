(function () {
  ...
  EvoClass.prototype = {
    ...
    generateSlickSlider: function () {
      let self = this;
      self.initSlick(
        $(".evo-box-slider:not(.slick-initialized)"),
        "box-slider"
      );
      self.initSlick(
        $(".evo-slider-half:not(.slick-initialized)"),
        "slider-half"
      );
      self.initSlick(
        $(".evo-slider:not(.slick-initialized)"),
        "product-slider"
      );
      ...
      document.querySelectorAll(".slick-lazy").forEach(function (slickItem) {
        ...
        slickItem.addEventListener(
          ...
          function (e) {
            let mainNode = $(this);
            if (
              !mainNode.hasClass("slick-initialized") &&
              Math.abs(startX - e.changedTouches[0].pageX) > 80
            ) {
              mainNode.removeClass("slick-lazy");
              mainNode
                .find(".product-wrapper")
                .removeClass("mx-auto ml-auto-util mr-auto");
              self.initSlick(mainNode, mainNode.data("slick-type"));
              let slickOptions = mainNode.slick("getSlick");
              if (
                slickOptions.slideCount >
                mainNode.slick("slickGetOption", "slidesToShow")
              ) {
                let goTo;
                $.each(
                  slickOptions.originalSettings.responsive,
                  function (key, value) {
                    if (
                      value.breakpoint === slickOptions.activeBreakpoint &&
    ...
    scrollStuff: function () {
      ...
      if (sidePanel.length) {
        breakpoint =
          sidePanel.position().top + sidePanel.hiddenDimension("height");
      }
    ...
    productTabsPriceFlow: function () {
      ...
      if ($("#tab-link-priceFlow").length) {
        ...
        $("#tab-link-priceFlow").on("shown.bs.tab", function () {
          if (
            typeof window.priceHistoryChart !== "undefined" &&
            window.priceHistoryChart === null
          ) {
          ...
          .then(function () {
            ...
            if (recaptcha.length === 1) {
              ...
              if (typeof siteKey !== "undefined") {
                ...
                newRecaptcha
                  .attr("id", "popup-recaptcha")
                  .addClass("g-recaptcha form-group");
    ...
    smoothScroll: function () {
      ...
      $(document).delegate('a[href^="#"]', "click", function (e) {
        ...
        if (!e.isDefaultPrevented()) {
          ...
          if (
            that.smoothScrollToAnchor(elem.getAttribute("href"), supportHistory)
          ) {
            e.preventDefault();
          }
        }
    ...
    checkout: function () {
      ...
      $('input[name="Versandart"]', "#checkout-shipping-payment").on(
        "change",
        function () {
          ...
          var url =
            $("#jtl-io-path").data("path") +
            "/bestellvorgang.php?kVersandart=" +
            id;
          $.evo.loadContent(
            url,
            function () {
      ...
      $("#country").on("change", function (e) {
        ...
        $.evo
          .io()
          .call("checkDeliveryCountry", [val], {}, function (error, data) {
    ...
    loadContent: function (url, callback, error, animation, wrapper) {
      var that = this;
      var $wrapper =
        typeof wrapper === "undefined" || wrapper.length === 0
          ? $("#result-wrapper")
          : $(wrapper);
        ...
        $.evo
          ...
          .call(
            ...
            function (error, data) {
              ...
              var $wlPrivate = $(
                  'span[data-switch-label-state="private-' +
                    data.response.wlID +
                    '"]'
                ),
                $wlPublic = $(
                  'span[data-switch-label-state="public-' +
                    data.response.wlID +
                    '"]'
                ),
    ...
    redirectToNewPriceRange: function (priceRange, redirect, $wrapper) {
      ...
      let redirectURL = $.evo.updateURLParameter(currentURL, "pf", priceRange);
      if (redirect) {
      ...
      $.evo
        ...
        .call(
          ...
          function (error, data) {
            ...
            $('[data-review-id="' + review.kBewertung + '"]').removeClass(
              "on-list"
            );
            item.addClass("on-list");
            $(
              '[data-review-count-id="hilfreich_' + review.kBewertung + '"]'
            ).html(review.nHilfreich);
            $(
              '[data-review-count-id="nichthilfreich_' +
                review.kBewertung +
                '"]'
            ).html(review.nNichtHilfreich);
          }
        );
    },
    ...
    initWishlist: function () {
      ...
      if ($(wlFormID).length) {
        ...
        $(".js-update-wl").on("change", function () {
          $.evo
            .extended()
            .updateWishlistItem($(this).closest(".productbox-inner"));
        });
      ...
      function setWishlistItemheights() {
        $(".product-list")
          ...
          .each(function () {
            $(this).css(
              "height",
              window.innerWidth > globals.breakpoints.xl
                ? $(this).height()
                : "unset"
            );
          });
