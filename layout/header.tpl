    ...
    <head>
		...
		{block name='layout-header-head-resources-crit-outer'}
            <style id="criticalCSS">
                ...
                {/block}
                {* FOUC-Schutz: Mobile-Menü erst nach CSS nicht offen durchblitzen *}
                @media (max-width: 991.98px) {
                    header #mainNavigation.navbar-collapse:not(.show),
                    ...
                    header #mainNavigation.navbar-collapse.show,
        ...
        <script>(function(){
           ...
           {else}
                ...
                if (has_starting_point === false) {
                    should_render_link = false;} else if (product_id === 0) {
        ...
        {* Microsoft Clarity – nur nach Statistik-/Analyse-Consent (EU Cookie / Consent Manager) *}
        {literal}
        <script>
        (function () {
            var CLARITY_ID = 'rmxrjsuuj9';
            var loaded = false;
            ...
            function truthyConsent(value) {
                return value === true || value === 1 || value === '1' || value === 'granted' || value === 'accept' || value === 'accepted';
            }
            ...
            function hasAnalyticsConsent(detail) {
                ...
                if (detail.settings && typeof detail.settings === 'object') {
                    if (hasAnalyticsConsent(detail.settings)) {
                        return true;
                    }
                }
                if (truthyConsent(detail.analytics_storage)
                    || truthyConsent(detail.analytics)
                    || truthyConsent(detail.statistics)
                    || truthyConsent(detail.statistik)
                    || truthyConsent(detail.statistic)) {
                    return true;
                }
                var key;
                for (key in detail) {
                    ...
                    if (!truthyConsent(detail[key])) {
                        continue;
                    }
                    if (/analytics|statistik|statistic|google.?analytics|ga4|clarity/i.test(key)) {
                        return true;
                    }
                }
                return false;
            ...
            function tryFromConsentManager() {
                try {
                    if (!window.CM || typeof window.CM.getSettings !== 'function') {
                        return false;
                    }
                    var settings = window.CM.getSettings();
                    if (typeof settings === 'boolean') {
                        return false;
                    }
                    if (hasAnalyticsConsent(settings)) {
                        loadClarity();
                        return true;
                    }
                    var ids = ['statistics', 'statistik', 'analytics', 'google-analytics', 'google_analytics'];
                    var i;
                    for (i = 0; i < ids.length; i++) {
                        if (window.CM.getSettings(ids[i]) === true) {
                            loadClarity();
                            return true;
                        }
                    }
            ...
            function onConsentEvent(e) {
                if (hasAnalyticsConsent(e && e.detail)) {
                    loadClarity();
                    return;
                }
                tryFromConsentManager();
            }

            document.addEventListener('consent.ready', onConsentEvent);
            document.addEventListener('consent.updated', onConsentEvent);

            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', function () {
                    window.setTimeout(tryFromConsentManager, 0);
                });
            } else {
                window.setTimeout(tryFromConsentManager, 0);
            }
...
{if !$bExclusive}
    ...
    {block name='layout-header-header'}
        ...
        {block name='layout-header-branding-top-bar'}
            <div id="meta-bar" class="topbar-wrapper {if $nSeitenTyp === $smarty.const.PAGE_BESTELLVORGANG}d-none{else}d-flex{/if} {if $Einstellungen.template.megamenu.header_full_width === 'Y'}is-fullwidth{/if}">
                <div class="container-fluid {if $nSeitenTyp === $smarty.const.PAGE_BESTELLVORGANG}d-none{else}d-flex{/if}">
                                        ...
                                        {block name='layout-header-branding-shop-nav'}
                                            {nav id="shop-nav" right=true class="nav-right order-lg-last nav-icons"}
                                                ...
                                                {block name='layout-header-branding-shop-nav-include-language-dropdown'}
                                                    {include file='snippets/language_dropdown.tpl' dropdownClass='d-flex'}
                                                {/block}
                                                {if $isMobile}
                                                    {include file='layout/meta_bar.tpl'}
                                                {/if}
                                            ...
                                            <div id="mainNavigation" class="navbar-collapse nav-scrollbar">
                                                ...
                                                {block name='layout-header-include-include-categories-body'}
                                                    <div class="nav-mobile-body">
                                                        ...
                                                        {if $startIdCategories != 1563 && $startIdCategories != 217 && $startIdCategories != 310 && $startIdCategories != 1837}
